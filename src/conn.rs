...
use crate::{config::service::ServiceConfig, ctx::ConnContext, http11, protocols::Protocol, ws};
...
pub async fn on_conn<R: tokio::io::AsyncRead + Unpin, W: tokio::io::AsyncWrite + Unpin>(
    ...
    let r = tokio::io::BufReader::with_capacity(config.tcp.read_stream_buf_size.0, r);
    let w = tokio::io::BufWriter::with_capacity(config.tcp.read_stream_buf_size.0, w);
