...
impl Headers {
    ...
    pub fn set_content_type(&mut self, val: &str) { self.set("content-type", val) }
                ...
                match self.get_all(key) {
                    ...
                    Some(vec) => {
                        ...
                        for item in vec.iter() {
                            ...
                            let fo = item.split(',').map(|v| { v.trim() }).find(|&x| x.starts_with("deflate") || x.starts_with("gzip"));
                            if let Some(v) = fo {
                                if v.starts_with('d') {
                                    ct = Some(CompressType::Deflate);
                                } else {
                ...
                match self.get_all("transfer-encoding") {
                    ...
                    Some(vec) => {
                        for item in vec.iter() {
                            ...
                            let fo = item.split(',').map(|v| { v.trim() }).find(|&x| x.starts_with("chunked"));
                            if fo.is_some() {
                                v = true;
                                break;
                            }
