...
fn all_std_status_codes() -> Vec<&'static str> {
    vec![
        ...
        "407 Proxy Authentication Required", "408 Request Timeout", "409 Conflict", "410 Gone", "411 Length Required", "412 Precondition Failed", "413 Payload Too Large",
        "414 URI Too Long", "415 Unsupported Media Type", "416 Range Not Satisfiable", "417 Expectation Failed", "418 I'm a teapot", "422 Unprocessable Entity",
        "425 Too Early", "426 Upgrade Required", "428 Precondition Required", "429 Too Many Requests", "431 Request Header Fields Too Large", "451 Unavailable For Legal Reasons",
...
async unsafe fn get_reasons_map() -> &'static mut RwLock<HashMap<u32, String>> {
    while _REASONS_MAP.is_none() {
        let exchange_result = _REASONS_MAP_INIT_LOCK.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst);
        if exchange_result == Ok(false) {
            let mut map: HashMap<u32, String> = HashMap::new();
            for item in all_std_status_codes() {
                if let Some(idx) = item.find(' ') {
                    map.insert(item[..idx].parse::<u32>().unwrap(), item[idx + 1..].to_string());
                }
            }
...
pub async fn register_status_code(code: u32, reason: &str) {
    unsafe {
        assert!(!(_REASONS_MAP_FREEZE_LOCK.load(Ordering::SeqCst)), "can not register after first read");
    }
